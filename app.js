...
const GAME_LOCATION_DETAILS = {
    ...
    lexington: { name: 'Lexington, KY', match: /lexington|eureka springs/i, lat: 37.997554, lng: -84.456367, drive: { miles: 166, hours: 3.2 } },
    atlanta: { name: 'Atlanta, GA', match: /atlanta|roswell rd/i, lat: 33.855264, lng: -84.382682, drive: { miles: 197, hours: 3.9 } },
    littleRock: { name: 'Little Rock, AR', match: /little rock|bowman rd|arkansas skatium/i, lat: 34.741551, lng: -92.40681, drive: { miles: 535, hours: 9.7 } },
    antioch: { name: 'Antioch, TN', match: /antioch|hickory hollow/i, lat: 36.052002, lng: -86.667104, drive: { miles: 179, hours: 3.4 } },
    nashville: { name: 'Nashville, TN', match: /nashville|25th ave/i, lat: 36.149486, lng: -86.810166, drive: { miles: 181, hours: 3.4 } }
};
...
function simplifyTitle(value = '') {
    return value
        .replace(/^South\s+East\s+Beast\s+15u(?:\s+A)?\s*-\s*/i, '')
        .replace(/^Southeast\s+Beast\s+15u(?:\s+A)?\s*-\s*/i, '')
        .replace(/^South\s+East\s+Beast\s+15u(?:\s+A)?\s+/i, '')
        .replace(/^Southeast\s+Beast\s+15u(?:\s+A)?\s+/i, '')
        .replace(/\b15u\b/gi, '15U')
        .trim();
}
        ...
        if (line === 'END:VEVENT') {
            ...
            if (start) {
                parsed.push(enrichEvent({
                    ...
                    type: /camp/i.test(item.SUMMARY || '') ? 'camp' : 'u15'
                }));
            }
            item = null;
            continue;
...
async function loadSchedule() {
    ...
    try {
        ...
        if (!events.some(event => event.date.startsWith(currentMonthKey))) {
            ...
            const focusEvent = events.find(event => event.date >= todayKey) || events[0];
            if (focusEvent) {
                currentYear = Number(focusEvent.date.slice(0, 4));
                currentMonth = Number(focusEvent.date.slice(5, 7)) - 1;
            }
        }
        status.textContent =
